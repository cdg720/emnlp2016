...
flags.DEFINE_integer('silver', 2, 'silver')
...
def run_epoch(session, m, data, eval_op, verbose=False):
  ...
  for step, (x, y) in enumerate(
          self_train_reader2.ptb_iterator(data, m.batch_size, m.num_steps)):
    fetches = []
...
def run_epoch2(session, m, nbest, eval_op, eos, verbose=False):
  ...
  for step, (x, y, z) in enumerate(
          self_train_reader2.ptb_iterator2(data, m.batch_size, m.num_steps,
                                           nbest['idx2tree'], eos)):
    fetches = []
    fetches.append(m.cost)
    fetches.append(eval_op)
...
def test():
  ...
  _, _, _, valid_nbest_data, vocab \
    = self_train_reader2.ptb_raw_data(FLAGS.data_path)
...
def train():
  ...
  raw_data = self_train_reader2.ptb_raw_data(FLAGS.data_path)
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    silver_generator = self_train_reader2._file_to_word_ids3(silver_path)
    j = 0
